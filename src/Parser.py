...
class Parser:
	...
	def getFramaRow(args):
		...
		with open(filePath, 'r') as fp:     
			lines = fp.readlines()[idxStart:idxEnd]			
			for ln in lines:
				value = search(r'(\=|\:)(\s\d*\.?\d+)', ln)  #    \d+\.?\d+   RIPARTIRE DA QUA

				if value: content.append(value.group(2))  # print('Value: ' + value)
	...
	def getInputsRow(self, args, values = []):
		...
		with open(filePath, 'r') as fp:
			...
			# print(self.headers)
			# print(content)
			self.writeFile({key:value for key, value in zip(self.headers, content)})  # value
	...
	def inputParser(self, inputsPath):
		...
		params = [filesPath, 2, None, r'[a-z\[\]]+\s=']  # r'[a-z\[\]]+\s='
